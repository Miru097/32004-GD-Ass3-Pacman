...
public class LevelGenerator : MonoBehaviour
{
    ...
    private void UpperLeft()
    {
        ...
        {
            upperLeft[i].transform.Rotate(0f, 0f, -90.0f);
        }
        ...
        {
            upperLeft[i].transform.Rotate(0, 0, 180.0f);
        }
    }
    private void UpperRight()
    {
        ...
        for (int row = 0; row < Row; row++)
        {
            for (int column = 0; column < Column; column++)
            {
                upperRight[i] = Instantiate(gameObjects[levelMap[row, column]], new Vector2(x - column, y - row), Quaternion.identity);
                if (i<=210) i++;
            }
        }
